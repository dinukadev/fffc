...
public class ProcessDataFileServiceImplTest {
    ...
    @Test(expected = FixedFileFormatConverterException.class)
    public void shouldFailIfInvalidDateDataType() throws Exception {
        File dataFile = new File(Thread.currentThread().getContextClassLoader().getResource("invalid_date_data.txt").getFile());
        processDataFileService.processDatafile(metadata, csvPath, dataFile);
    }
}